...
def increase_audio_volume():
    """Increase system audio volume to 100% using amixer for all audio cards"""
    try:
            ...
            for card in audio_cards:
                for control in control_names:
                    try:
                        ...
                        cmd = ['amixer', '-c', card, 'set', control, '100%']
                        result = subprocess.run(cmd, capture_output=True, timeout=3, check=False)
                        if result.returncode == 0:
                            print(f"Set card {card} {control} to 100%")
                    except Exception:
                        pass  # Skip if control doesn't exist on this card
            
            # Also try the generic Master control
            subprocess.run(['amixer', 'set', 'Master', '100%'], timeout=5, check=False)
            
            print("Audio volume configuration completed")
        else:
...
ENABLE_FRAME_ROTATION = True  # Set to False to eliminate expensive rotation
...
def get_emotion_response(frame, products):
    ...
    try:
        ...
        prompt_text = f"根据客户的情绪推荐喝的，一个短句子回答。可推荐的产品：{products_str}"
        ...
        try:
            ...
            if faces and application_should_run and not is_greeted:
                ...
                if GREET_GENDER_ENABLED:
                    if args.llmgender:
                        # Use LLM for gender detection
                        gender = get_llm_gender(frame)
                        print(f"LLM Gender detected: {gender}")
                    else:
                        # Use InsightFace for gender detection
                        gender = parse_gender(closest_face)
                        
                        # Debug detection results - ENHANCED DEBUGGING
                        print(f"InsightFace Gender detected: {gender}")
                ...
                # Queue the greeting and mark as greeted
                success = queue_speech(greeting_text, 'instant_greeting', priority=0)
                if success:
                    is_greeted = True  # Mark as greeted to prevent duplicate greetings
    ...
    try:
        while True:
            ...
            if face_detected and faces:
                ...
                face_center = ((largest_face.bbox[0] + largest_face.bbox[2]) // 2, 
                              (largest_face.bbox[1] + largest_face.bbox[3]) // 2)
            ...
            if face_detected:
                ...
                # Greeting logic - now handled by face detection worker for immediate response
                # This logic is kept as backup but shouldn't trigger due to instant greeting
                if application_should_run and not is_greeted and not distance_too_far and closest_face_distance <= distance_threshold:
                    # Backup greeting call (shouldn't be needed with instant greeting)
                    print(f"Backup greeting triggered for gender: {closest_face_gender}")
