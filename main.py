...
def increase_audio_volume(volume_percent=100):
    """Set system audio volume (cross-platform)"""
    import platform
    ...
    try:
        ...
        if platform.system() == 'Windows':
            ...
            try:
                ...
                '@
                $speakers = (New-Object -comObject MMDeviceEnumerator).GetDefaultAudioEndpoint(0,0)
                $volume = $speakers.Activate([IAudioEndpointVolume],0,$null,[ref]$null)
                $volume.SetMasterVolumeLevelScalar({volume_percent/100.0},0)
                """
                subprocess.run(['powershell', '-Command', ps_command], 
                             capture_output=True, timeout=5, check=False)
                print(f"Windows audio volume set to {volume_percent}%")
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(f"Linux audio volume configuration completed at {volume_percent}%")
                else:
                    print(f"Failed to get mixer controls: {result.stderr}")
            except FileNotFoundError:
                print("amixer command not found - Linux audio volume adjustment skipped")
...
def set_default_audio_device():
    """Set the default audio device (cross-platform)"""
    import platform
    
    try:
        if platform.system() == 'Windows':
            # Windows: Audio device selection is handled by PyAudio automatically
            print("Windows: Default audio device handled by PyAudio")
        else:
            # Linux: Create or update ALSA configuration to prefer USB audio
            alsa_conf = """
pcm.!default {
    type hw
    card 0
...
# Emotion detection functions now handled by qwenai.py module
# get_emotion_response and get_llm_gender are imported from qwenai
