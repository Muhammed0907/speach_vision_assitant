...
class RealtimeMp3Player:
    ...
    def _find_audio_device(self):
        """Find a working audio output device, with Windows and Linux compatibility"""
        import platform
        ...
        for i in range(self._player.get_device_count()):
            try:
                ...
                if device_info['maxOutputChannels'] > 0:
                    ...
                    if is_windows:
                        # Windows: prioritize speakers, headphones, default devices
                        if any(term in name for term in ['speakers', 'headphones', 'realtek', 'windows directsound']):
                            device_priorities.insert(0, (i, device_info))  # High priority
                        elif 'default' in name or device_info.get('defaultSampleRate'):
                            device_priorities.insert(1, (i, device_info))  # Medium priority
                        else:
                            device_priorities.append((i, device_info))  # Normal priority
                    else:
                        # Linux: prioritize USB audio devices
